...
@app.route("/")
def index():
    sql = "SELECT c.id as id, c.title AS title, c.composer AS composer, c.views AS views, g.genre AS genre, c.notation AS NOTATION, AVG(d.difficulty) AS difficulty, AVG(r.rating) AS rating FROM compositions c, ratings r, genres g, difficultyratings d WHERE r.song_id=c.id AND d.song_id=c.id and g.song_id=c.id"
